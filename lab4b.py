...
def join_lists(l1, l2):
    ...
    j_temporary_set = set(l1).union(set(l2))
    new_list = list(j_temporary_set)
    return new_list

def match_lists(l1, l2):
    #match_lists will return a list that contains all values found in both l1 and l2
    m_temporary_set = set(l1).intersection(set(l2))
    new_list = list(m_temporary_set)
    return new_list

def diff_lists(l1, l2):
    #diff_lists will return a list that contains all different values, which are not shared between the lists
    d_temporary_set = set(l1).symmetric_difference(set(l2))
    new_list = list(d_temporary_set)
    return new_list
